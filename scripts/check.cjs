...
if(!fs.readFileSync(path.join(root,'LICENSE.md'),'utf8').includes('FRESHCUP-NONCOMMERCIAL-LICENSE.md')) throw Error('Fresh Cup noncommercial scope is missing from LICENSE.md');
...
for(const route of routes) {
 ...
 if(!(open<fresh && fresh<close)) throw Error(`${route}: Fresh Cup is outside the shared main content`);
