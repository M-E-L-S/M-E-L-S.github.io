// Canvas UI / David Haz — see CanvasUI-LICENSE.md and README.md.
(() => {
const DEFAULTS                             = {
  color: [0.31, 0.54, 1],
  intensity: 0.5,
...
function createFlameWrap(
  ...
  if (htmlInCanvas) {
    paintable.onpaint = () => {
      try {
        ...
        console.warn("FlameWrap HTML capture failed:", error);
        source.dispatchEvent(new CustomEvent("flamecaptureerror"));
      }
    };
  }
  ...
  function uploadContent() {
    ...
    try {
      ...
      console.warn("FlameWrap texture upload failed:", error);
      source.dispatchEvent(new CustomEvent("flamecaptureerror"));
      return;
    }
    if (!captureReported) {
      captureReported = true;
      source.dispatchEvent(new CustomEvent("flamecaptured"));
    }
...
window.CanvasUIFlameWrap = { createFlameWrap, supportsHtmlInCanvas };
})();
