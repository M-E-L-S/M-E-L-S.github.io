...
void main() {
  ...
  vLocal = p;
  vDisplacement = length(position - aHome);
  mat4 localFromView = inverse(modelViewMatrix);
  float pointWidth = gl_PointSize / (uDpr * uViewportHeight) * 2.0 * (-mv.z) / projectionMatrix[1][1];
  vPointRight = (localFromView * vec4(pointWidth, 0.0, 0.0, 0.0)).xyz;
  vPointUp = (localFromView * vec4(0.0, pointWidth, 0.0, 0.0)).xyz;
...
uniform bool uHasTextMask;
uniform vec4 uMaskBounds;
uniform vec2 uMaskAspect;
out vec4 outColor;
...
void main() {
  ...
  float r2 = dot(c, c);
  if (r2 > 0.25) discard;
  if (uHasTextMask && vDisplacement < 0.012) {
    vec3 local = vLocal + c.x * vPointRight - c.y * vPointUp;
    vec2 uv = (local.xy * uMaskBounds.z + uMaskBounds.xy) * uMaskAspect + 0.5;
    if (any(lessThan(uv, vec2(0.0))) || any(greaterThan(uv, vec2(1.0)))) discard;
    if (texture(uTextMask, uv).a < 0.5) discard;
  }
...
function sampleImage(data           , count        )              {
  ...
  for (let i = 0; i < data.width * data.height; i++) {
    ...
    // Preserve thin strokes; the fragment shader clips the entire point footprint.
  ...
  for (let i = 0; i < count; i++) {
    const pick = ((i + Math.random()) / count) * totalWeight;
    let lo = 0;
...
function normalizeCloud(sample             ) {
  ...
  sample.bounds = new THREE.Vector4(cx, cy, 1 / inv, 0);
  for (let i = 0; i < p.length; i += 3) {
    p[i] = (p[i] - cx) * inv;
    p[i + 1] = (p[i + 1] - cy) * inv;
    p[i + 2] = (p[i + 2] - cz) * inv;
...
export function createParticleObject(
  ...
  const { canvas, interactionTarget = canvas } = elements;
  ...
  const material = new THREE.ShaderMaterial({
    ...
    uniforms: {
      ...
      uHasTextMask: { value: false },
      uMaskBounds: { value: new THREE.Vector4() },
      uMaskAspect: { value: new THREE.Vector2(1, 1) },
  ...
  function buildCloud() {
    ...
    let count = Math.max(Math.round(config.count), 16);
    if (assetSource.kind === "image" && config.imageParticleDensity > 0) {
      const pixels = assetSource.data.data;
      let inkArea = 0;
      for (let i = 3; i < pixels.length; i += 4) {
        if (pixels[i] >= 128) inkArea += pixels[i] / 255;
      }
      count = Math.max(count, Math.min(config.maxImageParticles || count,
        Math.round(inkArea * config.imageParticleDensity)));
    }
    const canMorph = Boolean(config.morph && points);
    if (count === builtCount && points && !canMorph) return;
    builtCount = count;
    if (!canMorph) clearPoints();

    const sample =
      assetSource.kind === "mesh"
        ? sampleMesh(assetSource.scene, count)
        : sampleImage(assetSource.data, count);
    if (assetSource.kind === "image" && !config.normalizeImage) {
      // Keep the source image's coordinate system, including transparent margins.
      // This gives text a stable font size instead of stretching every short line.
      sample.bounds = new THREE.Vector4(0, 0, 1, 0);
    } else {
      normalizeCloud(sample);
    }
    material.uniforms.uTextMask.value?.dispose();
    material.uniforms.uTextMask.value = null;
    material.uniforms.uHasTextMask.value = assetSource.kind === "image";
    if (assetSource.kind === "image") {
      const data = assetSource.data;
      const texture = new THREE.DataTexture(data.data, data.width, data.height);
      texture.flipY = true;
      texture.needsUpdate = true;
      material.uniforms.uTextMask.value = texture;
      material.uniforms.uMaskBounds.value.copy(sample.bounds);
      const longest = Math.max(data.width, data.height);
      material.uniforms.uMaskAspect.value.set(longest / data.width, longest / data.height);
    }

    if (canMorph) {
      if (particleCount !== count) {
        // Reuse the moving cloud when density changes, instead of snapping to new text.
        const geometry = points.geometry;
        const oldPositions = geometry.getAttribute("position").array;
        const oldSeeds = geometry.getAttribute("aSeed").array;
        const order = Array.from({ length: particleCount }, (_, index) => index)
          .sort((a, b) => homes[a * 3] - homes[b * 3] || homes[a * 3 + 1] - homes[b * 3 + 1]);
        const positions = new Float32Array(count * 3);
        const resizedHomes = new Float32Array(count * 3);
        const resizedVelocities = new Float32Array(count * 3);
        const seeds = new Float32Array(count);
        for (let i = 0; i < count; i++) {
          const from = order[Math.min(particleCount - 1, Math.floor(i * particleCount / count))];
          positions.set(oldPositions.subarray(from * 3, from * 3 + 3), i * 3);
          resizedHomes.set(homes.subarray(from * 3, from * 3 + 3), i * 3);
          resizedVelocities.set(velocities.subarray(from * 3, from * 3 + 3), i * 3);
          seeds[i] = oldSeeds[from];
        }
        ...
        homes = resizedHomes;
        velocities = resizedVelocities;
        particleCount = count;
      }
      const oldOrder = Array.from({ length: count }, (_, index) => index)
        .sort((a, b) => homes[a * 3] - homes[b * 3] || homes[a * 3 + 1] - homes[b * 3 + 1]);
      const newOrder = Array.from({ length: count }, (_, index) => index)
        .sort((a, b) => sample.positions[a * 3] - sample.positions[b * 3]
          || sample.positions[a * 3 + 1] - sample.positions[b * 3 + 1]);
      const nextHomes = new Float32Array(sample.positions.length);
      const nextColors = new Float32Array(sample.colors.length);
      const nextShades = new Float32Array(sample.shades.length);
      for (let rank = 0; rank < count; rank++) {
        const oldIndex = oldOrder[rank];
        const newIndex = newOrder[rank];
        nextHomes.set(sample.positions.subarray(newIndex * 3, newIndex * 3 + 3), oldIndex * 3);
        nextColors.set(sample.colors.subarray(newIndex * 3, newIndex * 3 + 3), oldIndex * 3);
        nextShades[oldIndex] = sample.shades[newIndex];
      }
      homes = nextHomes;
      points.geometry.setAttribute("aHome", new THREE.BufferAttribute(nextHomes, 3));
      points.geometry.setAttribute("aColor", new THREE.BufferAttribute(nextColors, 3));
      points.geometry.setAttribute("aShade", new THREE.BufferAttribute(nextShades, 1));
      for (let i = 0; i < velocities.length; i += 3) {
        velocities[i] += (Math.random() - 0.5) * 0.018;
        velocities[i + 1] += (Math.random() - 0.5) * 0.018;
    ...
    positionAttr.setUsage(THREE.DynamicDrawUsage);
    geometry.setAttribute("position", positionAttr);
    geometry.setAttribute("aHome", new THREE.BufferAttribute(sample.positions.slice(), 3));
  ...
  async function loadAsset() {
    ...
    try {
      ...
      if (kind === "glb" || kind === "gltf") {
        ...
        const data = await rasterizeImage(blob, config.rasterSize);
        if (disposed || token !== loadToken) return;
        if (config.morph && points) {
          if (assetSource?.kind === "mesh") disposeObject(assetSource.scene);
        } else {
  ...
  function onPointerMove(event              ) {
    // Layout/entrance animations can move the canvas without a resize or scroll.
    const rect = canvas.getBoundingClientRect();
    pointerX = (event.clientX - rect.left) * canvas.clientWidth / Math.max(rect.width, 1);
    pointerY = (event.clientY - rect.top) * canvas.clientHeight / Math.max(rect.height, 1);
  ...
  interactionTarget.addEventListener("pointercancel", onPointerLeave, { passive: true });
  ...
  function simulate(delta        ) {
    ...
    // Reduced motion disables ambient animation, not deliberate pointer input.
  ...
  return {
    ...
    destroy() {
      ...
      interactionTarget.removeEventListener("pointercancel", onPointerLeave);
      controls.dispose();
      clearAsset();
      material.dispose();
      material.uniforms.uTextMask.value?.dispose();
