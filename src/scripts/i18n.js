(function () {
    ...
    Object.assign(english, {
        ...
        '搜索模式':'Search mode','标准模式':'Standard mode','并行模式':'Parallel mode','Worker 数量':'Worker count','搜索力度':'Search effort','少':'Low','中':'Medium','多':'High','极多':'Very high',
        '⚠ 并行模式会使用 CPU 多线程同时计算。Worker 越多，CPU 负载越大，设备可能发热或出现卡顿。':'⚠ Parallel mode uses multiple CPU threads at once. More Workers increase CPU load and may cause heat or lag.',
        '检测到本机有 {count} 个可用逻辑线程；搜索力度会按照实际情况自动调整总节点数。':'Detected {count} available logical threads on this device. Search effort automatically adjusts the total node count as needed.',
        '浏览器未报告逻辑线程数，暂按 {count} 个提供选项；搜索力度会自适应调整搜索宽度。':'The browser did not report its logical thread count. Showing {count} Worker options; search effort adjusts width automatically.',
        '{workers} 个 Worker：已展开 {expanded} 个节点，生成 {generated} 个不同状态。':'{workers} Workers: expanded {expanded} nodes and generated {generated} distinct states.',
        ...
        '自动探索':'Auto Decode','支持的编码与密码':'Supported Encodings and Ciphers','全自动多层解密工具':'Automatic Multi-layer Decoder','看懂一切暗语。 *本标语是产品目标，不是效果保证。':'Make sense of every secret message. *This is a product goal, not a guarantee.','待分析文本':'Text to analyze','粘贴各类编码、密码甚至多层嵌套密码……':'Paste an encoding, cipher, or even a multi-layer puzzle…','最大深度':'Maximum depth','节点预算':'Node budget','请选择尽可能接近的加密深度。最大深度越高，工具越倾向于舍弃看似明文的中间结果。':'Choose a depth as close as possible to the number of encryption layers. Higher depths make the tool more likely to disregard intermediate results that appear to be plaintext.','开始解密':'Start decoding','停止':'Stop','启用 ACG Name':'Enable ACG Name','？可能的ACG Name：':'Possible ACG Name? ',
    ...
    Object.assign(english, {
        ...
        '未显式分词时，较长的连续英文解码结果会结合独立的完整英语词表和字符组合尝试自动分词，并优先保留较长的完整词。':'Long decoded English letter runs without explicit word breaks are segmented using a separate complete English word list and character patterns, favoring longer complete words.',
        '编码（固定映射）':'Encodings (fixed mapping)','密码（需要密钥）':'Ciphers (key required)','特殊领域':'Specialized','数学与代码':'Math and code',
        '21 项':'21 entries','3 项':'3 entries','1 项':'1 entry','暂未收录':'Coming later','后续增加这一类时会在这里列出。':'New decoders in this category will appear here.','ACG Name 词库命中':'ACG Name dictionary match','数据来源：':'Data source: ','使用 Bangumi Archive 中具有罗马字或英文名、简体中文名的角色。开启后才加载角色词库；完整姓名参与自动分词，单独姓或名至少三个字母且使用较低权重。结果中出现可能的姓名时，会显示对应的 Bangumi 页面；重名和姓名片段只显示罗马音。':'Uses characters in Bangumi Archive with a romanized or English name and a Simplified Chinese name. The character dictionary loads only when enabled. Full names participate in automatic segmentation; isolated name parts must have at least three letters and carry lower weight. Unique full names link to Bangumi, while ambiguous names and name parts show romanization only.',
        '按标准之字形栅栏还原，枚举 2–10 栏；短文本不会枚举超过字符数一半的栏数。字母、数字、空白及各种可打印编码符号都作为字符参与重排；含数字或符号时逐级降低探测置信度。':'Decodes the standard zigzag rail fence with 2–10 rails, capped at half the character count for short texts. Letters, digits, whitespace, and printable encoding symbols all participate in the transposition. Detection confidence decreases in steps when digits or symbols appear.',
        ...
        '每 8 位还原一个字节并按 UTF-8 解码。支持 0/1 和任意两种不同字符表示二进制位；自定义字符组可用空格分隔。':'Decodes each eight bits to a byte and reads UTF-8. Supports 0/1 or any two distinct symbols; custom groups may be separated by spaces.',
        '每 8 位还原一个字节并按 UTF-8 解码。支持 0/1 或任意两种字符表示二进制位；字节组之间只用空白分隔，也可连续书写。':'Decodes each eight bits to a byte and reads UTF-8. Supports 0/1 or any two symbols; byte groups use whitespace or may be written continuously.',
        '每 8 位还原一个字节并按 UTF-8 解码。支持 0/1 或任意两种字符；字节组用单个空格分隔或连续书写，单词用 /、|、换行或多个空格分隔。':'Decodes each eight bits to a byte and reads UTF-8. Supports 0/1 or any two symbols. Use one space between byte groups, or write them continuously; /, |, a newline, or multiple spaces separate words.',
        '将空格或常见标点分隔的八进制字节还原为 UTF-8 文本。':'Decodes octal bytes separated by spaces or common punctuation into UTF-8 text.',
        '每五个二元符号映射到 A–Z。支持 A/B，也会尝试任意两种不同符号及相反的 A/B 对应方式。':'Maps each five binary symbols to A–Z. Supports A/B and tries either assignment for any two distinct symbols.',
        '每五个二元符号映射到 A–Z。支持 A/B 或任意两种符号；每五位之间只用空白分隔，也可连续书写。':'Maps every five binary symbols to A–Z. Supports A/B or any two symbols; five-symbol groups use whitespace or may be written continuously.',
        '每五个二元符号映射到 A–Z。支持 A/B 或任意两种符号；字母组用单个空格分隔或连续书写，单词用 /、|、换行或多个空格分隔。':'Maps every five binary symbols to A–Z. Supports A/B or any two symbols. Use one space between letter groups, or write them continuously; /, |, a newline, or multiple spaces separate words.',
        '支持拉丁字母与数字。标准点划可用空白分字、斜杠分词；也会尝试三种符号分别表示点、划和字符间隔。':'Supports Latin letters and digits. Standard Morse uses spaces between letters and slashes between words; three-symbol variants are also tested.',
        '支持拉丁字母与数字。点、划可用任意两种符号表示；字母之间固定用空白分隔，单词之间固定用 / 分隔。':'Supports Latin letters and digits. Any two symbols may represent dot and dash; whitespace separates letters and / separates words.',
        '支持拉丁字母与数字。点、划可用任意两种符号表示；字母用单个空格分隔，单词用 /、|、换行或多个空格分隔。':'Supports Latin letters and digits. Any two symbols may represent dot and dash; one space separates letters, while /, |, a newline, or multiple spaces separate words.',
        '无密钥标准方阵，I/J 共用一格；每两个 1–5 数字表示行和列。':'Uses a standard unkeyed 5×5 square with I/J sharing a cell. Each pair of digits from 1 to 5 gives a row and column.',
        '使用标准 5×5 方阵将 A、D、F、G、X 字母对映射为文字，不处理需要密钥的列换位层。':'Maps A, D, F, G, X pairs through the standard 5×5 square. Keyed columnar transposition is not decoded.',
        '每组两串点分别表示方阵行列，组间用逗号、斜杠或双空格分隔；C/K 共用一格。':'Two dot groups give the square row and column. Separate pairs with commas, slashes, or double spaces; C/K share a cell.',
        '每组两串重复字符分别表示方阵行列，点或其他字符均可；组间用逗号、斜杠或双空格分隔，C/K 共用一格。':'Two runs of repeated symbols give the square row and column. Dots or other symbols work; separate pairs with commas, slashes, or double spaces. C/K share a cell.',
        '每组两串重复字符分别表示方阵行列，点或其他字符均可；行列之间固定用一个空格、字母之间固定用 / 分隔，C/K 共用一格。':'Two runs of repeated symbols give the square row and column. Dots or other symbols work; use one space within a letter and / between letters. C/K share a cell.',
        '每两串敲击组成一个字母，点或其他重复字符均可，敲击串之间用单个空格分隔；单词用 /、|、换行或多个空格分隔，C/K 共用一格。':'Each pair of tap runs forms a letter. Dots or other repeated symbols work; one space separates runs, while /, |, a newline, or multiple spaces separate words. C/K share a cell.',
    ...
    Object.assign(englishFragments, {'可打印字符 ':'Printable characters ','全文可读覆盖 ':'Whole-text readable coverage ','熵 ':'Entropy ','英文词库命中 ':'English dictionary matches ','英文字符组合 ':'English letter patterns ','自动分词':'Automatic word segmentation','查看 ':'View ',' 介绍':' reference'});
    Object.assign(english, {
